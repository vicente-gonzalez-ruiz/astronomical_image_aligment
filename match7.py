...
for root, dirs, files in os.walk("./images/"):
    ...
    for filename in files:
        ...
        try:
            transf, (source_list, target_list) = aa.find_transform(source = source_luma, target = target_luma, max_control_points = MAX_CONTROL_POINTS)
            ...
            output, max, min = normalize(output)
            print(max, min)
            print(f"Writting output_{counter-2}.tif")
            cv.imwrite(f"output_{counter-2}.tif", output)
